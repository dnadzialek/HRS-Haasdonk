...
code = re.sub(
    ...
    r'''      let dir = new THREE.Vector3(0,0,-1).applyQuaternion(camera.quaternion);
...
code = re.sub(
    ...
    r'''if (passing && window.passVelocity) {
...
code = re.sub(
    ...
    r'''  <div id="joystick-zone-right"><div id="joystick-knob-right"></div></div>
...
code = re.sub(
    ...
    r'''    #joystick-zone-right { position: absolute; bottom: 30px; right: 30px; width: 120px; height: 120px; border-radius: 50%; background: rgba(255,255,255,0.2); border: 2px solid rgba(255,255,255,0.5); touch-action: none; display: none; z-index: 15; }
...
code = re.sub(
    ...
    r'''      #joystick-zone-right { bottom: 120px; }
...
code = re.sub(
    ...
    r"document.getElementById('joystick-zone-right').style.display = 'block';",
    code
)
...
code = re.sub(
    ...
    r'let touchMoveID = null, touchLookID = null, lastLookPos = {x: 0, y: 0}, isTapping = false; let joystickLookDelta = {x: 0, y: 0};',
    code
)
...
code = re.sub(
    r'function updateJoystick\(x, y\) \{[\s\S]*?maxD; \}\n\s*joyKnob\.style\.transform[^}]+}',
    r'''      function updateJoystick(x, y, zone, knob, isLook) {
...
code = re.sub(
    ...
    r'updateJoystick(t.clientX, t.clientY, joyZone, joyKnob, false)',
    code
)

code = re.sub(
    r'let dx = t\.clientX - lastLookPos\.x[\s\S]*?isTapping = false;\s*\}',
    r'updateJoystick(t.clientX, t.clientY, joyZoneRight, joyKnobRight, true); }',
    code
)
...
code = re.sub(
    ...
    r"joystickLookDelta = {x: 0, y: 0}; joyKnobRight.style.transform = 'translate(0px, 0px)';",
    code
)
